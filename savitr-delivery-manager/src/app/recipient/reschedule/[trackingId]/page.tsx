...
export default function ReschedulePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/recipient/deliveries/${trackingId}/reschedule`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to reschedule delivery");
      }
  ...
  if (!delivery.isEligibleForReschedule) {
    return (
      <div className="min-h-screen flex flex-col">
        <Header />
        <main className="flex-grow flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-red-600">Not Eligible for Rescheduling</h1>
            <p className="mt-2 text-gray-600">
              This delivery is not eligible for rescheduling. Rescheduling is only available 
              for upcoming deliveries and must be done before 9:30 AM on the day of delivery.
            </p>
            <Button
              className="mt-4"
              onClick={() => router.push("/recipient/dashboard")}
